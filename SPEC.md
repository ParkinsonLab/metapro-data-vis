...
Metapro Viz is an Electron desktop application that ingests RPKM (Reads Per Kilobase Million) output from [MetaPro](https://github.com/ParkinsonLab/MetaPro) — a metatranscriptomics pipeline — and renders five interactive visualizations of the relationship between **taxonomy** and **enzymatic / pathway annotation**: Overview, Krona (sunburst), Chord, Network, and Graph (3D scatter).
...
|---|---|---|
| Main (Node) | `src/main/` | TSV parsing, in-memory data store, SQLite-backed taxonomy/pathway lookups, all heavy aggregation |
| Preload | `src/preload/index.ts` | Exposes the default `@electron-toolkit/preload` `electronAPI` on `window.electron` (no custom API) |
| Renderer (React 19) | `src/renderer/src/` | Zustand store, navigation, d3/Plotly visualizations |

**Data flow:** Renderer calls `request(channel, params)` from `src/renderer/src/ipc.ts`, which sets `isLoading: true` and dispatches `ipcRenderer.send('request-${ch}', params)` → main process handler runs synchronously inside a `try/catch` envelope → main process replies with `event.reply('response-${ch}', { ok: true, value } | { ok: false, error })` → renderer handler in `App.tsx` clears `isLoading`, surfaces `last_error` on `ok: false`, or routes `value` to the per-channel handler that writes into Zustand → component reads from store and (re)draws.

Every channel goes through this envelope; before PR 2 a thrown handler would silently leave the renderer hanging on `isLoading: true` forever.
...
- **Key columns** (constants in `src/main/utils.ts`): `EC#, GeneID, Length, Reads, RPKM, Unclassified` — pass through untouched in shape.
...
### 3.3 In-memory state (main process)

Stored as module-level `let` bindings in `src/main/data_functions.ts`:
...
## 4. IPC API (main ↔ renderer)

All channels follow the pattern `request-${name}` (renderer → main) / `response-${name}` (main → renderer), registered in `src/main/index.ts`. Every reply is wrapped in an envelope:

```ts
type IPCEnvelope<T> = { ok: true; value: T } | { ok: false; error: string }
```

The `Channel` union in `src/renderer/src/ipc.ts` is the single source of truth for valid channel names; both `request(...)` and the response handlers in `App.tsx` are typed against it, so adding a channel without updating both sides is a compile error.

| Channel | Handler | Params | Returns (`value`) |
|---|---|---|---|
| `handshake` | `initialize()` | — | `0` ok / `3` DB missing |
| `load` | `add_data` | `{ name, data }` (TSV string) | the loaded `name` |
| `load_test` | `add_test_data` | — | `string[]` of fixture fnames |
| `overview` | `parse_overview` | `{ names }` | `{ counts_data, ann_data }` |
| `counts` | `parse_counts` | `{ names, tax_rank, selected_taxon, selected_ann_cat }` | count vector keyed on tax_rank |
| `krona` | `parse_krona` | `{ names, tax_rank, selected_taxon }` | hierarchical taxonomy tree |
| `chord` | `parse_ec_chord` | `{ names, tax_level, ann_level, selected_ann_cat, selected_taxon }` | `{ count_matrix, index, colors, tax_map, ann_map }` |
| `network` | `parse_network` | `{ names, tax_level, selected_taxon, pathway_name, width, height }` | `{ nodes, edges, colors }` — `pathway_name` is the readable name; `parse_network` resolves it to a pathway id internally |
| `pathway_list` | `parse_pathway_list` | `{ superpathway }` | `string[]` of pathway names within the superpathway |
...
- `last_error: string | null` — most recent IPC error surfaced through the envelope; rendered by `<ErrorBanner />` in `App.tsx` until the user dismisses it.

### 6.2 Routing in `App.tsx`

A NavBar swaps `mainState`. On mount, the renderer calls `request('handshake', undefined, { silent: true })` and registers permanent `response-${ch}` handlers (one per `Channel`) that:
...
|---|---|---|---|
| **Upload** | plain inputs | mounted | File picker + label input → `request('load', ...)`; "Load Test Files" → `request('load_test')`; pair-select dropdowns → `request('chord', ...)`. All three sites go through the typed `request(...)` helper (no more raw `ipcRenderer.send`) |
| **Overview** | `d3-pie` | mounted | Two pies (`counts` and `ann`) over phylum / superpathway; auto-fetches `request('overview', { names })` on mount and on `selected_file_list` change. `get_color` is inlined locally so the renderer never reaches into `src/main/utils.ts` |
...
```bash
npm install
npm start          # electron-vite preview (production preview)
npm run dev        # electron-vite dev --watch (HMR)
npm run build:mac  # / build:win / build:linux — package via electron-builder
npm test           # vitest run
```

The DB is **not** shipped with the source: per README, the released installer fetches it; otherwise it is built locally from the notebooks under `resources/scripts/`.

The module-level debug harness in `src/main/data_functions.ts` (which calls `initialize()` + `add_test_data()` + `parse_ec_chord(...)`) is gated behind `RUN_HARNESS=1`. To execute it manually:

```bash
RUN_HARNESS=1 npx tsx src/main/data_functions.ts
```
...
| Where | Issue |
|---|---|
| `src/main/data_functions.ts` (`get_fname`) | `substring(-4)` is a no-op; `.tsv` is **not** stripped from registered fnames. Currently masked because every call site passes the suffixed name; will need a real fix when downstream code starts assuming "base name" |
...
| `src/main/db_functions.ts` (`get_parents_at_level`) | The `t_${rank}` column name is interpolated into SQL. Currently safe (`rank` only ever flows in from a typed renderer literal), but the input boundary makes it worth allowlisting before any user-controlled value can reach this code path |
| `src/main/data_functions.ts` (`add_data`) | `data = { ...data, [name]: parsed }` is O(n²) in the number of loaded files; trivial to switch to direct mutation |
| `src/main/data_functions.ts` (`add_data` headers) | Header rename does N separate `SELECT name FROM names WHERE tax_id = ?` queries (one per taxonomy column). Batchable into one `WHERE tax_id IN (...)` lookup |
| `src/main/data_functions.ts` (bottom) | Module-level test harness still gated behind `RUN_HARNESS=1`. To be removed entirely when the data-context factory (PR 6) lands |
| `src/main/index.ts` | All `parse_*` calls run on the main process's event loop. A wide TSV upload or a fresh `parse_ec_chord` blocks the BrowserWindow for 10–30 s; user-perceived hangs are worked around today by the `isLoading` spinner only |

### 9.1 Architectural items queued for PR 6

These are intentionally deferred because they touch every IPC site at once:
...
- **Stringly-typed channels** → an `IPCContract` mapping each channel name to its `{ request, response }` pair, with `request<C>(...)` and `registerHandler<C>(...)` helpers replacing the current per-side handler registry.
- **Manual envelope** → migration to `ipcMain.handle` / `ipcRenderer.invoke`. Errors auto-reject; the wrapper added in PR 2 collapses into a thin shim that the typed contract covers.
