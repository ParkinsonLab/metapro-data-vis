...
|---|---|---|
| Parity bar | **Contract parity (B)** — same response fields/shape; Graph tab renders correctly on both backends. **Functional-only (C)** acceptable when justified, provided frontend stays backend-agnostic. | User constraint |
...
| EC ordering | Hierarchical alphabetical: `(superpathway, pathway, ec_normalized)` tuple, lexicographic | User constraint |
| Taxon ordering | Hierarchical alphabetical: full lineage tuple `(kingdom, phylum, class, order, family, genus, species, display_name)` lexicographic | User constraint |
| Outer index ordering | **Same rules as inner leaves** — categories are prefix segments of the pathway hierarchy (ann side) or taxonomy lineage (tax side), sorted by the same tuple rules | User constraint |
...
| Matrix builder | `analytics/api/graph_matrix.py` | Inner matrix, outer index, colors, zero-row trim |
...
| `ann_level` | Pathway rank used for `ec_map` category field and outer ann-axis categories |
...
Query `int_rpkm_by_ec_tax` directly — one row per `(ec_normalized, source_tax_id)`, no aggregation across redundant dimensions:

```sql
SELECT r.ec_normalized, r.source_tax_id, r.value
FROM int_rpkm_by_ec_tax r
WHERE r.value > 0
  AND <ann_filter_predicate via bridge_ec_pathway>
  AND <taxon_filter_predicate via bridge_tax_rollup>
```
...
Built from `bridge_ec_pathway`. Used by `graph_matrix.py` for:

1. EC ordering (pathway hierarchy sort keys)
2. Outer index annotation categories
3. Per-EC line colors (`get_sub_color` from category color)

Legacy `get_ec_map` reads the SQLite EC reference table; DuckDB uses the bridge Parquet. Values in the matrix come from triples, not from `ec_map`.
...
Built by joining `bridge_tax_rollup` (label at `tax_level`) with the display name (finest-rank / `names` lookup). Used by `Graph.tsx` for y-axis background surface grouping.

### 5.4 Lineage and pathway sort keys

**Pathway tuple** per EC (from `bridge_ec_pathway`):

| `ann_level` | Tuple used for EC leaf sort | Category prefix depth (outer ann axis) |
|---|---|---|
| `superpathway` | `(superpathway_name, ec_normalized)` | 1 segment: `superpathway_name` |
| `pathway` | `(superpathway_name, pathway_name, ec_normalized)` | 2 segments: through `pathway_name` |
| `pathway_node` | `(superpathway_name, pathway_name, ec_normalized)` | EC label is the node; categories at pathway depth |
...
| Segment | Sort key |
|---|---|
| ECs | Pathway tuple for `ann_level` (§5.4), lexicographic |
| Taxa | Full lineage tuple (§5.4), lexicographic |

Only ECs/taxa with at least one non-zero matrix cell appear after zero-row trim.
...
1. Build sorted `inner_matrix_index` from ordered EC and taxon label lists
2. Fill symmetric inner matrix from triples `(ec, display_tax_label, value)`
3. Apply gap fillers on `gap_1`, `gap_2`, `gap_3`
4. Trim zero-sum rows/columns (legacy `idx_to_keep` logic)
5. Build `outer_matrix_index` from sorted category lists (§6.2)
6. Assign colors (§6.3)
...
| Layer | Coverage |
|---|---|
| Unit | `test_graph_matrix.py` — matrix shape, gap fillers, zero-row trim, unified ordering, colors present |
...
| `ec_map` source | SQLite EC reference | `bridge_ec_pathway` Parquet |
...
- [ ] Value query reads `int_rpkm_by_ec_tax` — no double-counting from rollup redundancy
