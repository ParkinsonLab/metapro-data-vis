...
| Query + metadata | `analytics/api/graph_service.py` | Triples query (no bridge joins) + separate index/metadata queries |
...
**No bridge joins on the triples query.** Joining `bridge_ec_pathway` (one row per `pathway_node_id`) or `bridge_tax_rollup` (seven rows per taxon) would fan out and risk double-counting values. Filters are applied via `EXISTS` subqueries only:
...
Run **after** triples are fetched. Operate on distinct keys extracted from the triple result — bridge fan-out here affects metadata only, not summed values.

**EC axis** — distinct `ec_normalized` from triples, joined to `bridge_ec_pathway` for pathway sort keys:
...
JOIN bridge_ec_pathway b ON b.ec_normalized = t.ec_normalized
```

`bridge_ec_pathway` is one row per `(ec_normalized, pathway_node_id)`; dedupe to **one sort tuple per EC** (e.g. `DISTINCT ON (ec_normalized)` or pick the lexicographically smallest `(superpathway_name, pathway_name)` tuple when an EC maps to multiple pathway nodes).

From this result build:

- Sorted EC label list (inner index ann segment)
- `ec_map` entries (category at `ann_level` from bridge columns)
- Pathway sort keys (§5.5)

**Tax axis** — distinct `source_tax_id` from triples, joined to lineage PIVOT + `names` for display label and `tax_map`:
...
Built from the **EC metadata query** (§5.2) — one entry per EC present in filtered triples, not the full bridge. Used by `graph_matrix.py` for:

1. EC ordering (pathway hierarchy sort keys)
2. Outer index annotation categories (at `ann_level` depth — see §5.5)
...
|---|---|---|
| Ann categories | Distinct category labels at `ann_level` present in filtered data | Pathway tuple **truncated** to category depth (§5.5), lexicographic |
...
Port core logic from `parse_graph_data` / `make_inner_count_matrix` / `add_filler_value` in `src/server/parse.ts`. **`graph_matrix.py` receives triples + metadata from `graph_service` — it does not run SQL or join bridges.**

1. Build sorted `inner_matrix_index` from metadata query results (§5.2): ordered EC labels + ordered taxon display labels (§6.1)
2. Fill symmetric inner matrix by keyed lookup: for each triple `(ec_normalized, display_tax_label, value)`, place `value` at the corresponding matrix positions (no aggregation — triples are already unique per cell)
3. Apply gap fillers on `gap_1`, `gap_2`, `gap_3`
4. Build `outer_matrix_index` from sorted category lists (§6.2)
5. Assign colors (§6.3)

**No zero-row trim** on the DuckDB path — the inner index lists only ECs and taxa that appear as keys in filtered triples (§5.1); metadata queries (§5.2) attach sort keys to those same keys. Every index label has a corresponding triple by construction. (Legacy Express still trims; see §5.1 legacy note.)
...
| Service | `test_graph_service.py` — triples query has no bridge joins (row count = unique `(ec, source_tax_id)` pairs); metadata queries on distinct keys; shape on `fake_rpkm` fixture; filter narrowing; comparison-mode rejection |
