...
**Between-sample delta (future):** attach `runs/A/sample.duckdb` and `runs/B/sample.duckdb`; join marts on `(pathway_key, tax_key)`. Reference layout does not affect delta.
...
1. For each `(tax_id, requested_rank)` pair (cross `bridge_tax_rank_map` with the 7 ranks):
   - **Exact:** row exists where `rank = requested_rank` → `tax_key = rank_tax_id`, `tax_rank_resolved = requested_rank`.
   - **Fallback:** no exact row; take finest rank coarser than `requested_rank` (lowest rank_order ≤ requested) → `tax_key = rank_tax_id`, `tax_rank_resolved = that rank`.
   - **Unclassified:** no qualifying row → `tax_key = NULL`, `tax_rank_resolved = NULL`.
2. Join `names` on `tax_key` for `tax_label`; set `'Unclassified'` when `tax_key IS NULL`.
...
| `tax_key` | BIGINT | Resolved **ancestor** tax_id at `tax_rank_resolved`; distinct from `source_tax_id` which is the original sample taxon; NULL when Unclassified |
| `tax_rank_resolved` | VARCHAR | Actual rank used (may be coarser than `requested_rank` under fallback); NULL when Unclassified |
| `tax_label` | VARCHAR | Scientific name of `tax_key`; `'Unclassified'` when `tax_key` is null |
...
```sql
SELECT p.*, t.requested_rank, t.tax_key, t.tax_rank_resolved, t.tax_label
...
**Unclassified rows:** `source_tax_id` values absent from `bridge_tax_rollup` (or resolving to NULL) yield `tax_key = NULL`, `tax_label = 'Unclassified'`, `tax_rank_resolved = NULL`. Left join ensures these rows are preserved.

**NULL keys for gap rows:**

| Case | `tax_key` | `tax_label` | `pathway_key` | `pathway_label` |
|---|---|---|---|---|
...
| Condition | Meaning |
|---|---|
| `tax_key IS NULL` | Unclassified |
| `tax_rank_resolved = requested_rank` | Exact match |
| `tax_key IS NOT NULL` AND `tax_rank_resolved != requested_rank` | Coarser fallback |

Example: taxon A with `requested_rank = 'phylum'` resolves to Bacteroidota (exact); taxon B resolves to kingdom Pseudomonadati (fallback). Different `(tax_key, tax_rank_resolved)` pairs — no extra grouping dimension needed.
...
| `tax_key` | BIGINT | Resolved ancestor tax_id at `tax_rank_resolved`; distinct from `source_tax_id`; NULL when Unclassified |
| `tax_rank_resolved` | VARCHAR | Actual rank used (may be coarser than `requested_rank`); NULL when Unclassified |
| `tax_label` | VARCHAR | Scientific name of `tax_key`; `'Unclassified'` when null |
...
**Mart GROUP BY:** `(pathway_key, pathway_label, tax_key, tax_label, tax_rank_resolved, sample_id, pathway_level, CASE WHEN pathway_key IS NOT NULL THEN NULL ELSE ec_normalized END)`.
...
Different source tax_ids that resolve to the same `(tax_key, tax_rank_resolved)` aggregate together — regardless of whether they arrived via exact or fallback path. Different fallback targets (e.g. Bacteroidota vs Pseudomonadati) remain separate rows.
...
| `tax_key` | BIGINT | Resolved tax_id; NULL when Unclassified |
| `tax_label` | VARCHAR | Scientific name; `'Unclassified'` when null |
...
| `mart_classified_taxa_have_keys` | No row where `tax_label != 'Unclassified'` AND `tax_key IS NULL` | mart | error | 0 rows |
...
| `mart_rollup_exact_match_rate` | `SUM(value WHERE tax_rank_resolved = var('tax_rank') AND tax_label != 'Unclassified') / SUM(value)` | mart | info | Metric only |
| `mart_rollup_fallback_rate` | `SUM(value WHERE tax_rank_resolved != var('tax_rank') AND tax_label != 'Unclassified') / SUM(value)` | mart | info | Metric only |
| `mart_unclassified_rate` | `SUM(value WHERE tax_label = 'Unclassified') / SUM(value)` | mart | info | Metric only in v1 |
...
- **`mart_classified_taxa_have_keys`** — if we resolved a real taxon (`tax_label != 'Unclassified'`), `tax_key` must be non-null.
